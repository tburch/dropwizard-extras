...
mvn_release() {
    ...
    mvn_release_clean=$(mvn --settings ~/.m2/mySettings.xml -B release:clean)
    echo -n "'mvn -B release:clean' "
    mvn_release_prepare=$(mvn --settings ~/.m2/mySettings.xml -DautoVersionSubmodules=true -DupdateDependencies=false -B release:prepare)
    echo -n "'mvn -B release:prepare' "
    mvn_release_perform=$(mvn --settings ~/.m2/mySettings.xml release:perform)
...
test -n "--settings ~/.m2/mySettings.xml" && {
    echo "Maven arguments provided : --settings ~/.m2/mySettings.xml."
}
